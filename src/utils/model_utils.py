...
def load_gpt2_pretrained_weights(filepath, model):
    ...
    for var in model.trainable_variables:
        ...
        # Use var.path if available (var.name could be a leaf name)
        var_name = var.path if hasattr(var, 'path') else var.name

        # print()
        # print(var_name)

        # Replace the model name by 'gpt2_lm'
        parts = var_name.split('/')
        parts[0] = 'gpt2_lm'
        var_name = '/'.join(parts)

        # Add the ':0' suffix if it is not present
        if var_name[-2:] != ':0':
            var_name += ':0'
